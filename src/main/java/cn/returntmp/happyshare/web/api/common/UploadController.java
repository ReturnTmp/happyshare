package cn.returntmp.happyshare.web.api.common;
...
import cn.returntmp.happyshare.dto.LinkToImageUrlDTO;
...
import cn.returntmp.happyshare.util.FileUtils;
import cn.returntmp.happyshare.util.SpringContextHolder;
...
import cn.returntmp.happyshare.util.SSRFUtil;
...
public class UploadController {
    ...
    public GlobalResult<JSONObject> uploadPicture(@RequestParam(value = "file", required = false) MultipartFile multipartFile, @RequestParam(defaultValue = "1") Integer type, HttpServletRequest request) throws IOException {
        ...
        String fileUrl = happyshareFileService.getFileUrlByMd5(md5, tokenUser.getIdUser(), fileType);
        ...
        try {
            ...
            happyshareFileService.insertHappyshareFile(fileUrl, savePath, md5, tokenUser.getIdUser(), multipartFile.getSize(), fileType);
            data.put("url", fileUrl);
        } catch (IOException e) {
            data.put("message", "上传失败!");
        }
    ...
    public GlobalResult<Map<String, String>> linkToImageUrl(@RequestBody LinkToImageUrlDTO linkToImageUrlDTO, HttpServletRequest request) throws IOException {
        ...
        try (InputStream inputStream = conn.getInputStream()) {
            ...
            happyshareFileService.insertHappyshareFile(fileUrl, savePath, md5, tokenUser.getIdUser(), getData.length, fileType);
            data.put("originalURL", url);
            data.put("url", fileUrl);
            return GlobalResultGenerator.genSuccessResult(data);
        } catch (IOException e) {
